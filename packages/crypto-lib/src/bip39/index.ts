...
import englishJson from "./wordlists/english.json";

import { sha512 } from "@noble/hashes/sha512";

let DEFAULT_WORDLIST: string[] = englishJson;
...
export function mnemonicToSeed(mnemonic: string, password?: string): Promise<Buffer> {
  return Promise.resolve().then((): Promise<Buffer> => {
    const mnemonicBuffer = Buffer.from(normalize(mnemonic), "utf8");
    const saltBuffer = Buffer.from(salt(normalize(password)), "utf8");
...
export function getDefaultWordlist(): string {
  ...
  return Object.keys(wordlists).filter((lang: string): boolean => {
    ...
    return wordlists[lang].every(
      (word: string, index: number): boolean => word === DEFAULT_WORDLIST![index]
    );
  })[0];
}
