...
export function contextRandomize() {
  return 0;
}

export function privateKeyVerify(seckey: Uint8Array) {
  const bn = new BN(seckey);
  return bn.cmp(curve.n) < 0 && !bn.isZero();
}

export function privateKeyNegate(seckey: Uint8Array) {
  const bn = new BN(seckey);
  return Buffer.from(curve.n.sub(bn).umod(curve.n).toArray("be", 32));
}

export function privateKeyTweakAdd(seckey: Uint8Array, tweak: Uint8Array) {
...
export function privateKeyTweakMul(seckey: Uint8Array, tweak: Uint8Array) {
  let bn = new BN(tweak);
...
export function publicKeyVerify(pubkey: Uint8Array) {
...
export function publicKeyCreate(seckey: Uint8Array, compress: boolean) {
  const bn = new BN(seckey);
...
export function publicKeyConvert(pubKey: Uint8Array, compress: boolean) {
  const pair = loadPublicKey(pubKey);
  const point = pair?.getPublic();
  return Buffer.from(point.encode(null, compress));
}

export function publicKeyNegate(pubKey: Uint8Array, compress: boolean) {
  const pair = loadPublicKey(pubKey);
...
export function publicKeyCombine(pubKeys: Uint8Array[], compress: boolean) {
...
export function publicKeyTweakAdd(pubKeys: Uint8Array, tweak: Uint8Array, compress: boolean) {
...
export function publicKeyTweakMul(pubkey: Uint8Array, tweak: Uint8Array, compress: boolean) {
  const pair = loadPublicKey(pubkey);
  if (pair === null) return null;
...
export function signatureNormalize(sig: Uint8Array) {
...
export function signatureExport(obj: any, sig: Uint8Array) {
...
// Copied 1-to-1 from https://github.com/bitcoinjs/bip66/blob/master/index.js
// Adapted for Uint8Array instead Buffer
export function signatureImport(output: any, sig: Uint8Array) {
...
export function ecdsaSign(message: Uint8Array, seckey: Uint8Array) {
  const d = new BN(seckey);
  if (d.cmp(curve.n) >= 0 || d.isZero()) return null;
  return secp256k1.sign(message, seckey, { canonical: true }).toBytes();
}

export function ecdsaVerify(sig: Uint8Array, msg32: Uint8Array, pubkey: Uint8Array) {
...
export function ecdsaRecover(sig: Uint8Array, recid: number, msg32: Uint8Array, compress: boolean) {
