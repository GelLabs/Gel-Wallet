  ...
  while (true) {
    ...
    if (signUtil.ed25519.privateKeyVerify(randBytes)) {
      if (ed25519SignTest(randBytes)) {
        ...
        const privateKey: Uint8Array = concatPub
          ? base.concatBytes(randBytes, publicKey)
          : randBytes;
...
export async function ed25519_getDerivedPrivateKey(
  ...
  try {
    ...
    return encode === "base58"
      ? Promise.resolve(base.toBase58(privateKey))
      : Promise.resolve(base.toHex(privateKey));
  } catch (error) {
